...
*/
function make_allow_return_value(
  is_allowed,
  file,
  ace,
  explain_why,
...
function allow_user_action(
  ...
  if (
    ...
    ].includes(permission_to_check) &&
    (file.owner === user || admin_group.users.includes(user))
  ) {
    return make_allow_return_value(
      true,
      file,
      null,
      explain_why,
      "This user is the file owner or an administrator"
    );
  }
  ...
  return make_allow_return_value(
    ...
    null,
    explain_why,
    "No permissions found for this file and user"
  );
}
...
function emitState(purpose = "Permission state changed") {
  ...
  let data = new SpecialEventEntry(
...
function remove_permissions(file, user, permissions, is_allow) {
  file.acl = file.acl.filter((ace) => {
    return !(
      ace.who === user &&
      permissions.includes(ace.permission) &&
      ace.is_allow_ace === is_allow
    );
  });
  emitState();
}
